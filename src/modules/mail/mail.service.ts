...
export class MailService {
  ...
  async resetPassword(user: User) {
    const url = `${this.configService.get<string>('base_urls.front')}/user/reinitialisation?token=${user.reset_password_token}&user_id=${user._id}`;
  ...
  async sendUserEmailUpdate(user: User) {
    const url = `${this.configService.get<string>('base_urls.front')}/user/changementMail?token=${user.new_email_token}&user_id=${user._id}`;
